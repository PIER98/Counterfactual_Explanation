...
predictions = naiveBayes.predict(X_test) #predictions on test data
...
diceModel = dice_ml.Model(model=naiveBayes, backend="sklearn")
...
counterFactual = exp.generate_counterfactuals(query_instances=query_instance, total_CFs=5,desired_class="opposite")
